...
class Player:
    ...
    def from_url(cls, url: str, team: str, position: str, sleeper_id: str):
        ...
        return cls(name, team, position, rankings_table, adp_table, projections_table, stats_table, expert_note, sleeper_id, [news])
...
def get_pick(player_id, draft_id):
    ...
    df['sleeper_id'] = df['player_filename'].apply(lambda x: sleeper_id_map.get(x, None))
    ...
    input_str = "# Available Players\n" + "\n".join([str(p) for p in players]) + f"\n# Current Team\n{team_table}\nQuestion: Which player should I pick?"
    messages = [
        {
            "role": "system",
